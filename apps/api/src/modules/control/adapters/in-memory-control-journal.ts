...
import type { ControlReceipt } from "../domain/types.js";
import type {
  BeginControlDecision,
  ControlOperationJournal,
} from "../ports/control-journal.js";
...
type JournalRecord = {
  ...
  terminalFailure?: ControlErrorClass;
};

export class InMemoryControlOperationJournal implements ControlOperationJournal {
  ...
  begin(input: Parameters<ControlOperationJournal["begin"]>[0]): Promise<BeginControlDecision> {
  ...
  commitReceipt(input: Parameters<ControlOperationJournal["commitReceipt"]>[0]): Promise<ControlReceipt> {
    return this.#exclusive(async () => {
      const record = this.#records.get(scopeKey(input.context));
      if (record === undefined || record.operationRecordId !== input.operationRecordId) {
        throw new Error("Control operation was not initialized.");
      }
      if (record.receipt !== undefined) {
        if (record.receipt.receiptHash !== input.receipt.receiptHash) {
          throw new Error("Control receipt conflicts with the committed receipt.");
        }
      ...
      this.audit.push({
        action: "control.evidence.decided",
      ...
      this.outbox.push({
        aggregateId: input.context.claimId,
        eventType: "control.evidence.decided",
  ...
  async recordAttempt(input: Parameters<ControlOperationJournal["recordAttempt"]>[0]): Promise<void> {
    this.attempts.push({
  ...
  recordFailure(input: Parameters<ControlOperationJournal["recordFailure"]>[0]): Promise<void> {
    return this.#exclusive(async () => {
      const record = this.#records.get(scopeKey(input.context));
      if (record !== undefined) {
      ...
      this.audit.push({
        action: "control.evidence.failed",
        claimId: input.context.claimId,
